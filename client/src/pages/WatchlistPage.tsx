...
function RegimeBadge({ regime }: { regime: string }) {
  ...
  return (
    <span
      ...
      style={{ fontFamily: mono, fontSize: 10, color, background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)" }}
    >
      {regime}
    </span>
  );
...
function ConcentrationBadge({ type }: { type: string | null }) {
  if (!type) return <span style={{ fontFamily: mono, fontSize: 10, color: "rgba(255,255,255,0.12)" }}>—</span>;
  ...
  return (
    <span className="inline-block px-2 py-0.5 rounded-sm" style={{ fontFamily: mono, fontSize: 10, color: entry.textColor, border: `1px solid ${entry.borderColor}` }}>
      {entry.label}
    </span>
  );
}
...
export default function WatchlistPage() {
  ...
  return (
    <div className="px-6 py-6 min-h-screen" style={{ background: "#0f0f0f" }}>
      <div className="mb-5">
        <p className="text-[10px] tracking-[0.2em] uppercase mb-1" style={{ fontFamily: mono, color: "#38BDF8" }}>
          WATCHLIST
        </p>
      ...
      >
        <span className="text-sm text-white font-bold" style={{ fontFamily: mono }}>
      ...
      {watchlistedStocks.length === 0 ? (
        <div className="py-20 text-center" data-testid="watchlist-empty-state">
          <p className="text-4xl mb-4" style={{ color: "rgba(255,255,255,0.06)" }}>☆</p>
          <p className="text-sm mb-3" style={{ fontFamily: mono, color: "#6b7280" }}>
            Belum ada saham di watchlist
          </p>
          <Link href="/radar">
            <button
              className="px-4 py-2 rounded-sm transition-all text-xs"
              style={{
                fontFamily: mono,
          ...
          <table className="w-full min-w-[1000px]">
              ...
              <tr style={{ background: "#111111", borderBottom: "1px solid rgba(255,255,255,0.03)" }}>
                {["SAHAM", "SEKTOR", "SKOR", "REZIM", "POSISI SIKLUS", "ALIRAN DANA", "KONSENTRASI", "DITAMBAHKAN", "STATUS", "AKSI"].map((h) => (
                  <th
                    ...
                    style={{ fontFamily: mono, fontSize: 9, letterSpacing: "0.1em", color: "#6b7280", textTransform: "uppercase", fontWeight: 400 }}
                  >
                    {h}
                  </th>
                ))}
            ...
            <tbody>
              {watchlistedStocks.map((stock) => {
                ...
                const wItem = watchlistMap.get(stock.symbol);
                return (
                  <tr
                    key={stock.symbol}
                    ...
                    <td className="px-4 py-3 w-48">
                      <div className="flex items-center gap-3 py-1">
                        ...
                        <div className="flex flex-col gap-0.5">
                          <p className="text-base font-bold text-white" style={{ fontFamily: sora }}>{stock.symbol}</p>
                          <p className="text-[11px] truncate" style={{ fontFamily: mono, color: "#6b7280", maxWidth: 160 }}>{stock.name}</p>
                        </div>
                      </div>
                    </td>
                      ...
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <Link href={`/stock/${stock.symbol}`}>
                          <button
                            ...
                            style={{ fontFamily: mono, background: "rgba(56,189,248,0.1)", color: "#38BDF8", border: "1px solid rgba(56,189,248,0.2)" }}
