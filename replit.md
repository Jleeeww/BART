...
- `server/engine/bandarmologyCore.ts` — **Intelligence Layer v2.0.0**: `computeBandarmologyV2()` — 12-model institutional-grade engine with 5-phase pipeline. Models: M1 (AccumulationStrength), M2 (AbsorptionScore), M3 (BrokerDominance), M6 (FlowNormalized), M7 (BrokerRotation), M8 (StealthAccumulation), M9 (FakeBreakoutRisk), M11 (FlowDecomposition), M12 (RollingAccumulation), M14 (CampaignDuration), M15 (PriceElasticity), M16 (RegimeStability). Includes: liquidity tiers, calendar suppression (M13), composite scoring with regime modifier, data integrity validation. M16 reads historical M6 scores from storage (never today's).
- `server/engine/bandarmologyCoreV1.ts` — **Intelligence Layer v1.1.0** (legacy, used by existing routes): `computeBandarmology()` and `buildBandarmologyInput()` — 23-step engine. Also exports `computeGorenganFromStock()` for homepage use.
- `server/engine/buildBandarmologyInput.ts` — **v2.0 Input Adapter**: `buildBandarmologyInput()` — converts raw DB stock records into typed `BandarmologyInput` for v2.0 engine. Safe parsing for broker data, foreign activity, IDR values. Handles missing fields gracefully.
- `server/engine/bandarmologyHistory.ts` — **v2.0 History Storage**: `getM6History()` and `saveBandarmologyHistory()` — persistent M6 score storage for M16 RegimeStability. In-memory fallback, oldest→newest ordering. Max 20 records per symbol.
- `server/routes/testBandarmology.ts` — **v2.0 Test Endpoint**: `GET /api/test-bandarmology` — returns full BandarmologyV2Result with sample BBRI_TEST data, seeded M6 history for regime detection.
