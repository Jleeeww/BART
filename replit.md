...
- `server/engine/bandarmologyCore.ts` — **Intelligence Layer v1.1.0**: `computeBandarmology()` and `buildBandarmologyInput()` — consolidates ALL 23-step bandarmology intelligence into a single deterministic engine. Steps 1-18 (v1.0): GorenganDetector, BrokerControl, BrokerStability, TapeControl, BrokerClassification, MarketMode (A/D), ConvictionPhase, SmartMoneyIntent, TrapDetection, DecisionEngine, ControlQuality, InsiderAlignment, SimplifiedRisk, SmartMoneyReadiness, PhaseDetection. Steps 19-23 (v1.1): FlowNormalization (fix B-1: negative range), BrokerRotation (stub for multi-day data), StealthAccumulation (fix B-2: slight negative slope), FakeBreakoutRisk, RegimeProbabilities. Also exports `computeGorenganFromStock()` for homepage use. Utility: `clamp100()`, `buildLevelLabel()`, `parseBrokers()`. Type: `ScoreWithExplanation` interface.
