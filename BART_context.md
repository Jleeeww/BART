...
- **Claude Web Search Integration v1.0** (Session 15): Added real-time web search enrichment to `POST /api/ai`. Installed `@anthropic-ai/sdk@^0.93.0`. Added module-level `anthropicClient` (null-safe — only initialised when `ANTHROPIC_API_KEY` env var is set) and `aiAnalysisCache` Map (15-min TTL per symbol). In the `/api/ai` endpoint, after all deterministic engine computations (bandarmology, actionGuidance, valuation modifier, smartNewsFilter), added a `Promise.race(30s timeout)` Claude call using model `claude-sonnet-4-5` with `web_search_20250305` tool (max_uses: 3) and Bahasa Indonesia system prompt. Response extracts text-only blocks (filters tool_use blocks), caches result, and adds as `webSearchAnalysis: string | null` to the existing response JSON — all other fields unchanged. Frontend `StockDashboard.tsx` renders `webSearchAnalysis` as a new card at the top of the Berita tab when present (sky blue border, IBM Plex Mono label, "Analisis Live — Web Search"). Failures return `webSearchAnalysis: null` and never block the response. Calibration verified: all 4 engine tests pass, BBCA and UNVR base scores unchanged. Files modified: `server/routes.ts` (import + module-level cache + endpoint enrichment), `client/src/pages/StockDashboard.tsx` (Berita tab card), `package.json` (new dependency).

- **Sector Rotation Intelligence v1.0** (Session 14): Built additive 4-part system on Pasar page. (1) Created `server/engine/sectorRotationEngine.ts` — groups stocks by sector, computes weighted-average readiness score (top 5 stocks per sector with weights [3,2,1.5,1,1]), derives MASUK/KELUAR/NETRAL direction (score≥60+akumulasi-bias→MASUK, score<40+distribusi-bias→KELUAR), top 3 stocks per sector, momentum vs prior cache (NAIK if Δ≥+10, TURUN if Δ≤-10), dominantTheme synthesis. 15min market-hours / 60min off-hours cache; exposes `getCachedSectorRotation()` for cache-aware route fast path. (2) Created `server/engine/macroContext.ts` — fetches Yahoo Finance CL=F (oil), GC=F (gold), USDIDR=X with 6s timeout + AbortController; falls back to hardcoded values on failure. Generates macro signals from oil/gold/USDIDR/CPO/coal trends → mapped to sector effects (oil_surge→Energy POSITIF, gold_safe_haven→Financials NEGATIF, idr_weak→Consumer Staples NEGATIF, cpo_up→Consumer Staples POSITIF, coal_down→Energy NEGATIF). Computes RISK_ON/RISK_OFF/NETRAL market sentiment from positive vs negative signal balance. 60min cache with input-aware key (cpo+coal trends). (3) Appended `GET /api/sector-rotation` (cache fast-path: returns cached snapshot without fetching /api/stocks if engine cache warm; slow path fetches merged DB+fallback universe via internal /api/stocks call to include Energy/Industrials/Real Estate fallback stocks) and `GET /api/macro-context` (pulls cpo/coal trends from getAltDataSnapshot, calls getMacroContext) to routes.ts. (4) Modified `client/src/pages/PasarPage.tsx` — added typed interfaces (SectorRotationSnapshot, MacroContextSnapshot, MacroCommodity, MacroSignal), 2 useState + 1 useEffect with res.ok gating + shape validation. Section A (KONDISI MAKRO SAAT INI above DATA ALTERNATIF): sentiment bar with reason, 4-col commodity grid (oil/gold/USDIDR/CPO with source badges), signals list with sector-Bahasa mapping. Section B: replaced static SECTORS placeholders inside PERFORMA SEKTOR card with live sector tiles (color-coded borders MASUK=emerald/KELUAR=red/NETRAL=neutral, momentum arrows, Bahasa direction labels) + dominantTheme banner; falls back to "—%" placeholders if engine returns null. Section C (SAHAM UNGGULAN DI SEKTOR PANAS): renders only when hotSectors.length>0, shows top 3 stocks per hot sector (max 2 sectors) with company logos, color-coded scores, cycle position badges, DETAIL→ links. Verified: BBCA=74/WATCHLIST_PRIORITAS, UNVR=10/HINDARI_DULU intact. /api/sector-rotation returns 6 sectors. /api/macro-context returns LIVE oil=101.94 (+7.99%), gold=4629.9, usdIdr=17345 with 3 signals and RISK_ON sentiment. Code review by architect addressed: (a) sector-rotation cache bypass — added getCachedSectorRotation() with route fast path; (b) macro cache key ignored cpo/coal inputs — added input-aware cacheKey; (c) frontend `any` types and missing res.ok checks — added typed interfaces and response.ok + shape validation. e2e tests pass. Files created: `server/engine/sectorRotationEngine.ts`, `server/engine/macroContext.ts`. Files modified: `server/routes.ts` (append-only), `client/src/pages/PasarPage.tsx`, `replit.md`.

- **Distribution Early Warning System v1.0** (Session 13): Created `server/engine/distributionWarning.ts` — additive 5-condition detector (C1 campaign aging with sustained 2+ session decline, C2 M6 flow weakening, C3 broker side-switch with null-safety, C4 M15 elasticity rising, C5 M11 flow alignment diverging). Alert tiers: AMAN/PANTAU_DISTRIBUSI/WASPADA_DISTRIBUSI/BAHAYA_DISTRIBUSI. Loads last 5 sessions from session_history; safe fallback returns AMAN on DB errors. Added `GET /api/distribution/:symbol` endpoint to routes.ts (append-only, dynamic import, parseNum helper). Wired UI integration to 3 pages: StockDashboard.tsx (full warning card with conditions/recommendation, validates alertLevel before render), RadarPage.tsx (compact badge under stock name, parallel fetch via Promise.allSettled), WatchlistPage.tsx (badge inside STATUS column, dedicated useEffect fetching only watchlisted symbols). All UI elements render only when alertLevel !== 'AMAN'. Calibration verified intact: BBCA=74/WATCHLIST_PRIORITAS, UNVR=10/HINDARI_DULU. Code review by architect addressed: (a) WatchlistPage missing fetch — added; (b) C1 single-step decline — strengthened to 2+ consecutive sessions; (c) StockDashboard fetch hardening — added res.ok check + alertLevel validation. e2e tests pass on all 3 pages. Files created: `server/engine/distributionWarning.ts`. Files modified: `server/routes.ts`, `client/src/pages/StockDashboard.tsx`, `client/src/pages/RadarPage.tsx`, `client/src/pages/WatchlistPage.tsx`, `replit.md`.
...
- **Pre-Launch Validation Infrastructure** (Session 11): Three failure prevention systems + backtesting pipeline. (1) Gorengan Whitelist: 50 blue-chip IDX stocks auto-excluded from gorengan detection in gorenganDetector.ts — BBCA and UNVR now always isGorengan=false. Added effectiveVolumeRatio cap (2.5 max when top3>60% and vol>3x) to prevent false positives on institutional-heavy stocks. (2) Lifecycle Smoothing: Added diragudanSessions column to signal_lifecycle table. DIRAGUKAN via drift now requires 2+ consecutive sessions (first drift session stays AKTIF with warning). Prevents single-session noise from triggering status change. (3) Score Distribution Monitor: New scoreMonitor.ts analyzes engine score distribution (mean, median, std, bucket percentages, extreme rate). Health status: HEALTHY/WARNING/CRITICAL. Wired into radarEngine.ts — runs after each radar batch, caches result. (4) Backtesting Pipeline: New backtestEngine.ts reads session_history, identifies accumulation signals (score≥60 + accumulation regime), computes 5/10/15-session forward returns and accuracy. backtestAll() for multi-symbol summaries with calibration recommendations. (5) New endpoints: GET /api/monitor/scores, POST /api/backtest, GET /api/backtest/:symbol. Files created: scoreMonitor.ts, backtestEngine.ts. Files modified: gorenganDetector.ts, signalLifecycle.ts, schema.ts, radarEngine.ts, routes.ts (append only). Calibration: BBCA=74/WATCHLIST_PRIORITAS (not gorengan), UNVR=10/HINDARI_DULU (not gorengan).
- **Signal Lifecycle Persistence** (Session 11): Built signal lifecycle tracking system for watchlisted stocks. New `signal_lifecycle` table in schema.ts (symbol unique, status AKTIF/DIRAGUKAN/GUGUR, baseline/current scores, drift, reason). New `server/engine/signalLifecycle.ts` with `computeStatus()` (5-layer check: GUGUR permanent, score<35 GUGUR, WASPADAI_DISTRIBUSI cycle GUGUR, drift≥25 GUGUR, drift≥15 DIRAGUKAN, regime-based DIRAGUKAN for distribusi/spekulatif/volatile). `updateSignalLifecycle()` upserts on symbol, never deletes records. `GET /api/signals` joins watchlist×lifecycle×stocks. `POST /api/signals/:symbol/update` computes readiness via unified brain + valuation modifier (same logic as /api/stocks). WatchlistPage.tsx shows STATUS column (green AKTIF/amber DIRAGUKAN/red GUGUR badges with tooltip), warning bar when DIRAGUKAN/GUGUR exists, "Perbarui" button triggers bulk update via apiRequest+cache invalidation. Uses TanStack Query for signals fetch. Calibration verified: BBCA=74/AKTIF, UNVR=10/GUGUR. Files created: signalLifecycle.ts. Files modified: schema.ts, routes.ts (append only), WatchlistPage.tsx.
- **Gorengan Detector Extraction** (Session 10): Extracted gorengan detection logic from `bandarmologyCoreV1.ts` into standalone `server/engine/gorenganDetector.ts`. New module imports helpers from existing files (parseBrokerIDR, foreignParser, tapeControl). Three improvements applied: (1) Real volumeRatio from todayValue/avg20dValue instead of hardcoded 1.2, (2) Smarter smallLotDominates requires foreignFlowAbsent, (3) hasTapeControl graceful fallback when avgBuyPrice=0. `bandarmologyCoreV1.ts` now delegates to new module via re-export (backward compatible). Wired into `radarEngine.ts` as safety override: every stock runs through gorengan check independently of v2.0 engine output; gorengan stocks forced to hindari_dulu with compositeScore=0. Calibration verified: BBCA=74/not gorengan, UNVR=10/gorengan. Files created: gorenganDetector.ts. Files modified: bandarmologyCoreV1.ts, radarEngine.ts.
- **Radar Engine History Integration** (Session 10): Modified `server/engine/radarEngine.ts` to use `buildStockHistory()` from `historyBuilder.ts` instead of static DB fields. `processStock()` now dynamically imports historyBuilder, fetches up to 60 sessions, and merges into `enrichedRaw` (history takes priority over static fields for netFlowHistory, priceHistory, avg20dValue, _m6ScoreHistory). Falls back to raw record if no session_history data available — fully backward compatible. M6 history for M16 uses historyBuilder first, falls back to getM6History. `assessDataQuality()` now evaluates enrichedRaw. Added `historySource` (string) and `sessionCount` (number) fields to `RadarStockResult` interface. Calibration verified: BBCA=74/WATCHLIST_PRIORITAS, UNVR=10/HINDARI_DULU. Only `radarEngine.ts` modified.
- **IDX Data Ingestion Pipeline** (Session 10): Created full ingestion pipeline for session-level market data. Added `session_history` table to `shared/schema.ts` (30 columns: OHLCV, flow metrics, broker jsonb, engine scores, unique index on symbol+date+session). Created `server/engine/historyBuilder.ts` — `buildStockHistory()` reads up to 60 sessions for arrays (netFlow, price, m6Score), `saveSessionHistory()` upserts via onConflictDoUpdate, `getHistoryStatus()` returns session count + date range. Created `server/engine/idxIngester.ts` — `ingestSession()` processes IDXSessionPayload (date, session 0/1/2, source, stocks array), computes netFlow/changePct/flowSignals, saves each stock, invalidates radar cache on success; `seedDemoHistory()` for quick demo seeding. Added 2 endpoints: `GET /api/history/:symbol` (returns session count, date range, hasFullHistory/hasLongHistory), `POST /api/ingest` (Bearer token auth via INGEST_SECRET env var, validates payload, returns success/errors/skipped/durationMs). DB pushed via drizzle-kit. Files created: historyBuilder.ts, idxIngester.ts. Files modified: schema.ts, routes.ts.
