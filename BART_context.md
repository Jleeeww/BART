...
- **Radar batch engine + API** (Session 6): Created `server/engine/radarEngine.ts` — processes stocks in 50-stock batches, 15min/60min cache TTL, 5000ms per-stock timeout, 60% confidence gate. Added `GET /api/radar` and `GET /api/radar/status` endpoints to routes.ts. RadarPage.tsx rewired from `/api/stocks` (useQuery) to `/api/radar` (useState+useEffect fetch). Added loading spinner, error state, cache info bar (last update time + next refresh + stats), 15-minute auto-refresh interval. `hiddenCount` now driven by API response. Table fields updated: `name`→`companyName`, `readinessScore`→`compositeScore`, `marketRegime`→`regime`, `cyclePosition`/`concentrationType`/`flowBias` now from API (removed client-side `deriveV2`). Files modified: routes.ts, RadarPage.tsx. File created: radarEngine.ts.
